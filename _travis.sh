...
EOF
  ...
  git init
  git config user.name "jaredtao"
  git config user.email "wentaojia2014@163.com"
  git add .
  git commit -m "Update Blog By TravisCI With Build $TRAVIS_BUILD_NUMBER"
  # Github Pages
  git push --force --quiet "https://${GH_TOKEN}@${GH_REF}" master:master
  # Gitee Pages
  git push --force --quiet "https://jaredtao:${GITEE_TOKEN}@${GITEE_REF}" master:master
  ...
  # Gitee Pages
  git push --quiet "https://jaredtao:${GITEE_TOKEN}@${GITEE_REF}" master:master --tags
}
